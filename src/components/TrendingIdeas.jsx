...
export default function TrendingIdeas({
  ...
  return (
    <section className="max-w-7xl mx-auto">
      ...
      {loading ? (
        <div className="min-h-55 rounded-2xl border border-slate-200 bg-white" />
      ) : error ? (
          ...
          {ideas.map((idea) => (
            ...
            >
              ...
              <div className="flex h-full flex-1 flex-col p-5">
                ...
                <h3 className="mt-3 text-xl font-bold text-slate-900 line-clamp-2 min-h-14">
                  {idea.title}
                </h3>

                <p className="mt-3 text-sm text-slate-600 line-clamp-3 min-h-18">
                  {idea.shortDescription}
                </p>
                <div className="mt-3 flex items-center justify-between text-xs">
                  {/* Budget - Left side */}
                  <span className="inline-flex items-center gap-1 rounded-full bg-green-50 px-3 py-1 font-medium text-green-700 border border-green-100">
                    <FaMoneyBillWave className="text-green-600 text-xs" />
                    Budget: {idea.estimatedBudget || "Not listed"}
                  </span>

                  {/* Name - Right side */}
                  <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 font-medium text-blue-600 border border-green-100">
